        ...
        <div className="container-header-table">
            <div className="container-header-table-grid">
                ...
                <div className="container-header-table-section3">
                    {!!buttons.length &&
                        buttons.map((button: IButtonsProps, index: number) => {
                            return handleShowButtons(button, index)
                        })}
                    {showInvoicesOptions && (
                        <Select
                            placeholder="Pago Grupal..."
                            style={{ width: 160 }}
                            onSelect={onClickGrupal}
                        >
                            {userPermissions.indexOf(PERMISSIONS.UPLOAD_OC) >
                                -1 && (
                                <Option value="uploadgroupoc">
                                    Carga OC grupal
                                </Option>
                            )}
                            {userPermissions.indexOf(PERMISSIONS.CONFIRM_OC) >
                                -1 && (
                                <Option value="validategroupoc">
                                    Validar OC grupal
                                </Option>
                            )}
                            {userPermissions.indexOf(
                                ...
                                <Option value="uploadgroupinvoice">
                                    Carga factura grupal
                                </Option>
                            )}
                            {userPermissions.indexOf(
                                PERMISSIONS.CONFIRM_INVOICE
                            ) > -1 && (
                                <Option value="validategroupinvoice">
                                    Validar factura grupal
                                </Option>
                            )}
                        </Select>
                    )}
