...
const MapAbsenseToInsert = (absense: AusencesModel, idEmployee: string,  initdate: Moment, finishdate: Moment) => {
  const diffDays = finishdate.diff(initdate, 'days');
  const auxDays = (absense.abrev_ausencia === 'MD' || absense.abrev_ausencia === 'MR') ? diffDays/2 : diffDays;
  return {
    ...absense,
