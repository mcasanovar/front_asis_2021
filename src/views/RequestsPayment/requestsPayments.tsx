...
const RequestsPaymentView: React.FunctionComponent<
    IRequestsPaymentViewProps
> = ({ authorized }) => {
    ...
    const [requestpaymentSelected, setRequestpaymentSelected] =
    ...
    const handleCLickActionTable = (
        ...
        switch (id) {
            case 'details':
                ...
                idregister &&
                    setRequestpaymentSelected(
                        requestpayment.find(
                            request => request._id === idregister
                        )
                    )
                setOpenModal(true)
                break
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_REQUEST_PAYMENT.find(
    ...
    async function getRequestPayment(pagenumber: number) {
        setLoading(true)
        const aux: IResponseAllRequestPayment =
            await getAllRequestPaymentService(pagenumber, N_PER_PAGE)
        if (!aux.err) {
            setRequestpayment(aux.cobranzas)
            setActualPage(aux.pagina_actual)
            setTotalItems(aux.total_items)
        ...
        return setMessageAlert({
            message: 'No se ha podido cargar las cobranzas',
            type: 'error',
            show: true,
        })
    }
