...
import { Input, Row, Spin, Col, Form, Select, Table, Typography, Checkbox, DatePicker, Button } from 'antd';
...
const ConsolidatedReportView: React.FunctionComponent<IConsolidatedReportViewProps> = ({
  onCloseModal
}) => {
  const { Option } = Select;
  const { Column } = Table;
  ...
  const [showServiceNameFilter, setShowServiceNameFilter] = useState<boolean>(false);
  ...
  async function getRequetsPaymentByGI() {
    ...
    const requestPaymentMapped: RequestPaymentModel[] = res.map((element) => {
      return {
        ...element,
        fecha_cobranza: moment(element.fecha_facturacion, FORMAT_DATE).add(5, 'd').toDate()
      }
    });
  ...
  const handleFilterRequestPayment = () => {
    ...
    if (showContractFilter) {
      result = result?.filter((element) => element.faena_seleccionada_cp.toLowerCase() === faenaSelected?.toLowerCase());
    }
  ...
  const handleGetFaenas = (contractNumber: string) => {
    const aux: IContract | undefined = companySelected?.contrato_faenas.find((element) => element.nro_contrato === contractNumber);
    console.log(aux)
    if (!aux) return;
        ...
        <Col span={6}>
          <Form layout='vertical'>
            ...
            >
              ...
              />
              <Checkbox style={{ paddingLeft: '1rem' }} onChange={() => setShowDateFilter(!showDateFilter)}></Checkbox>
            </Form.Item>
          </Form>
        </Col>
        <Col span={10} style={{ display: 'flex', flexDirection: 'row', width: 'max-content' }}>
          <Form layout='vertical' style={{ width: '100%' }}>
            ...
            >
              ...
              >
                {!!companySelected && !!companySelected.contrato_faenas.length && companySelected.contrato_faenas.map((company, index) => (
                  <Option key={index} value={company.nro_contrato}>{company.nro_contrato}</Option>
                ))}
              </Select>
              <Select
                style={{ width: '40%' }}
                onSelect={(e: SelectValue) => setFaenaSelected(e.toString())}
                placeholder='Faena'
                disabled={!showContractFilter}
                value={faenaSelected || ''}
              >
                {!!contractSelected && !!contractSelected.faenas.length && contractSelected.faenas.map((contract, index) => (
                  <Option key={index} value={contract.name}>{contract.name}</Option>
                ))}
              </Select>
              <Checkbox style={{ paddingLeft: '1rem' }} onChange={() => setShowContractFilter(!showContractFilter)}></Checkbox>
            </Form.Item>
          </Form>
        </Col>
        <Col span={8}>
          <Form layout='vertical'>
            <Form.Item
              label='Filtro Nombre Servicio'
            >
              <Select
                style={{ width: '90%' }}
                onSelect={(e: SelectValue) => setServiceSelected(e.toString())}
                placeholder='Nombre Servicio'
                disabled={!showServiceNameFilter}
              >
                {!!servicesSelected.length && servicesSelected.map((service, index) => (
                  <Option key={index} value={service}>{service}</Option>
                ))}
              </Select>
              <Checkbox style={{ paddingLeft: '1rem' }} onChange={() => setShowServiceNameFilter(!showServiceNameFilter)}></Checkbox>
            </Form.Item>
          </Form>
        </Col>
      </>
  ...
  const renderTableExams = () => {
    return (
      ...
      >
        ...
        <Column className='column-money' title="Rut Evaluado" dataIndex="rut_cs" key="rut_cs" />
        <Column className='column-money' title="Razon Social Evaluado" dataIndex="razon_social_cs" key="razon_social_cs" />
        <Column
          className='column-money'
          ...
          render={(_, record: any) => {
            return <>
              <Text>{`$ ${MilesFormat(record.valor_servicio)}`}</Text>
            </>
          }}
        />
        <Column
        ...
        <Form layout='vertical'>
          <Row gutter={8}>
            <Col span={12}>
              <Form.Item
                label='Proveedor'
                validateStatus={!!companySelected ? 'success' : 'error'}
                help={!!companySelected ? '' : 'Seleccione proveedor'}
              >
                <Select
                  showSearch
                  ...
                  style={{ width: '100%' }}
                  onSelect={(e: SelectValue) => handleGetCompanySelected(e.toString())}
                >
                  {!!companies && !!companies.length && companies.map((company, index) => (
                    <Option key={index} value={company._id}>{company.razon_social}</Option>
                  ))}
                </Select>
              </Form.Item>
            </Col>
