...
const RequestView: React.FunctionComponent<IRequestViewProps> = ({
    authorized,
}) => {
    ...
    const handleCLickActionTable = (
        ...
        switch (id) {
            ...
            case 'sendmail':
                idregister &&
                    setSelectedRequest(
                        requests.find(request => request._id === idregister)
                    )
                setActualModal({
                    _id: id,
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_REQUEST.find(
    ...
    const handleChangePagination = (newpage: number) => {
        ...
        if (filterMode) {
            !filterObjectSelected &&
                filterRequests(filterSelected, 'fecha_solicitud', newpage)
            !!filterObjectSelected &&
                filterRequests(
    ...
    async function getRequests(pagenumber: number) {
        const aux: IResponseAllRequests = await getAllRequestsService(
            pagenumber,
            N_PER_PAGE
        )
        if (!aux.err) {
        ...
        return setMessageAlert({
            message: 'No se ha podido cargar las solicitudes',
            type: 'error',
            show: true,
        })
    }
