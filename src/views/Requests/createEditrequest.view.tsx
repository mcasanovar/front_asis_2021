...
const CreateRequestView: React.FunctionComponent<ICreateRequestViewProps> = ({
    ...
    async function getGIByRut(rut: string, typeRequest: number) {
        ...
        if (typeRequest === 1) {
            setNewRequestData({
                ...newRequestData,
                razon_social_CP: aux.res.razon_social,
                rut_CP: aux.res.rut,
                id_GI_Principal: aux.res._id,
            })
            setPrimaryClient(aux.res)
        } else {
            setNewRequestData({
                ...newRequestData,
                razon_social_cs: aux.res.razon_social,
                rut_cs: aux.res.rut,
                id_GI_Secundario: aux.res._id,
            })
            setSecondaryClient(aux.res)
        }
    ...
    const handleSetContractNumber = (e: string) => {
        setNewRequestData({
            ...newRequestData,
            nro_contrato_seleccionado_cp: e,
        })
                        ...
                        <Col span={6}>
                            <Form.Item label="Fecha solicitud">
                                <DatePicker
                                    ...
                                    onSelect={(e: Moment) =>
                                        handleSelectRequestDate(e, true)
                                    }
                                />
                            </Form.Item>
                        </Col>
