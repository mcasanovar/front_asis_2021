...
const GroupConfirmationView: React.FunctionComponent<
    IGroupConfirmationViewProps
> = ({ onCloseModal }) => {
    const { Option } = Select
    const { TextArea, Search } = Input
    const { Column } = Table

    const [dataConfirmation, setDataConfirmation] = useState<IDataConfirmation>(
        IDataConfirmationInitialization
    )
    ...
    const handleGroupConfirmRequests = async () => {
        ...
        const aux: IResponseRequest = await confirmGroupRequestsService(
            formData
        )
        if (!aux.err) {
                    ...
                    <Row gutter={8}>
                        ...
                        <Col span={6}>
                            <Form.Item
                                ...
                                validateStatus={
                                    dataConfirmation.medio_confirmacion !== ''
                                        ? 'success'
                                        : 'error'
                                }
                                help={
                        ...
                        <Col span={24}>
                            <Form.Item label="Observaciones">
                                <TextArea
                                    ...
                                    value={
                                        dataConfirmation.observacion_solicitud
                                    }
                                />
                            </Form.Item>
                        </Col>
    ...
    useEffect(() => {
        if (!dataConfirmation.medio_confirmacion)
