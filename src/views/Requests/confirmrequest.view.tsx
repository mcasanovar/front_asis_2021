...
const ConfirmRequestView: React.FunctionComponent<IConfirmRequestViewProps> = ({
    ...
    const [confirmRequest, setConfirmRequest] = useState<RequestModel>(
    ...
    const handleValidateEmail = (e: FormEvent<HTMLInputElement>) => {
        setIsPrimaryEmailOk(
            validateEmail(e.currentTarget.value) && e.currentTarget.value !== ''
        )
        setConfirmRequest({
            ...confirmRequest,
            email_gi: e.currentTarget.value,
        })
    }
    ...
    const handleConfirmRequest = async () => {
        ...
        const requestToConfirm = MapRequestToConfirm(
            confirmRequest,
            checkSendMail,
            arrayEmails
        )
        formData.append('data', JSON.stringify(requestToConfirm))
        const result: IResponseRequest = await confirmRequestService(
            confirmRequest._id,
            formData
        )
        if (result.err === null) {
        ...
        <Spin spinning={loading} size="large" tip="Cargando...">
            ...
            <Input.Group>
                <Form layout="vertical">
                    <Row gutter={8}>
                        <Col span={6}>
                            <Form.Item label="Código servicio">
                                <Input value={confirmRequest.codigo} readOnly />
                            </Form.Item>
                        </Col>
                        <Col span={6}>
                        ...
                        <Col span={6}>
                            <Form.Item label="Fecha confirmación">
                                <DatePicker
                                    ...
                                    style={{ width: '100%' }}
                                    value={
                                        confirmRequest.fecha_confirmacion !== ''
                                            ? moment(
                                                  confirmRequest.fecha_confirmacion,
                                                  FORMAT_DATE
                                              )
                        ...
                        <Col span={12}>
                            <Form.Item
                                ...
                                help={
                                    confirmRequest.medio_confirmacion !== ''
                                        ? ''
                                        : 'Seleccione medio de confirmación'
                                }
                            >
                                <Select
                                    style={{ width: '100%' }}
