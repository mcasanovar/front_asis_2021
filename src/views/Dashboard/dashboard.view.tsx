...
} from '../../functions/getLocalStorage'
...
const DashboardScreen: React.FunctionComponent<IDashboardScreenProps> = ({
    authorized,
}) => {
    const { TabPane } = Tabs
    const { Option } = Select

    const [monthAcumChecked, setMonthAcumChecked] = useState<{
    ...
    const handleGetAllResports = async (year: string = actualYear) => {
        ...
        if (result.err) {
            setMessageAlert({
                message:
                    'Se ha producido un error al cargar los datos. Por favor, intente mas tarde',
                type: 'error',
                show: true,
            })
            setloading(false)
    ...
    const calculateAcumulate = () => {
        ...
        const auxInvoices: number[] = calculateAcumulateArray(
            dataDashboard?.invoices || []
        )

        dataDashboard?.cashFlow.forEach(cashflowData => {
            const acumArrayCashflow: number[] = calculateAcumulateArray(
                cashflowData.data
            )
            auxCashFlow.push({
                type: cashflowData.type,
                data: acumArrayCashflow,
            })
        })
                                ...
                                datasets: [
                                    {
                                        label: 'Ingresos',
                                        data:
                                            dataDashboard?.cashFlow.find(
                                                element =>
                                                    element.type === 'ingresos'
                                            )?.data || [],
                        ...
                        <div className="container-simple-card-second-title-and-switch">
                            ...
                            <Switch
                                ...
                                onChange={() =>
                                    setMonthAcumChecked({
                                        monthChecked:
                                            !monthAcumChecked.monthChecked,
                                        acumChecked:
                                            !monthAcumChecked.acumChecked,
                                    })
                                }
                            />
                        </div>
                        {monthAcumChecked.monthChecked &&
                            renderEvolutionAndRankingMonth()}
                        {monthAcumChecked.acumChecked &&
                            renderEvolutionAndRankingAcum()}
                    </div>
                </Col>
                                    ...
                                    renderItem={(item, index) => (
                                        <List.Item>
                                            <List.Item.Meta
                                                ...
                                                title={item.name}
                                            />
                                            <h3 className="simple-card-text">{`$ ${MilesFormat(
                                                item.quantity
                                            )}`}</h3>
                                        </List.Item>
                                    )}
    ...
    return (
        <div className="container-dashboard">
            ...
            <div className="container-dropdown-year">
                ...
                {loading ? (
                    <Title level={4}>
                        Recargando datos...Esto puede tardar hasta 50
                        segundos...
                    </Title>
                ) : (
                    <h4></h4>
