...
> = ({ onCloseModal }) => {
    ...
    async function getResultsByGI() {
        setLoading(true)
        const aux: IResponseResults = await getResultsByRutGIService(
            companySelected?.rut || ''
        )
        ...
        const auxServices = res.reduce(
            (acc: string[], current: ResultModel) => {
    ...
    const handleFilterRequestPayment = () => {
        ...
        if (!showDateFilter && !showContractFilter && !showServiceNameFilter)
            return setResultsFiltered(results)
        ...
        if (showDateFilter) {
            ...
            result = results?.filter(element => {
                ...
                return (
                    element?.fecha_resultado_date &&
                    moment(element.fecha_resultado_date).isBetween(
                        dateValueRange[0],
                        dateValueRange[1]
                    )
                )
            ...
            let aux = results?.filter(
                element =>
                    ...
                    element.fecha_resultado ===
                        moment(dateValueRange[1]).format(FORMAT_DATE)
            )
    ...
    const handleGenerateReport = async () => {
        ...
        const aux: IResponseRequestPayment =
            await generateConsolidatedReportResultsService(dataMapped)
        if (!aux.err) {
