...
const ResultsView: React.FunctionComponent<IResultsViewProps> = ({
    authorized,
}) => {
    ...
    async function getResults(pagenumber: number) {
        setLoading(true)
        const aux: IResponseAllResults = await getAllResultsService(
            pagenumber,
            N_PER_PAGE
        )
        if (!aux.err) {
            const reservationsMapped = aux.resultados.map(reservation => {
                return {
                    ...
                    fecha_resultado: reservation.fecha_resultado
                        ? `${reservation.fecha_resultado} ${
                              reservation?.hora_resultado
                                  ? reservation.hora_resultado
                                  : ''
                          }`
                        : 'En Revisión',
        ...
        return setMessageAlert({
            message: 'No se ha podido cargar los resultados',
            type: 'error',
            show: true,
        })
    }
    ...
    const handleCLickActionTable = (
        ...
        switch (id) {
            ...
            case 'uploadresult':
                ...
                idregister &&
                    setResultSelected(
                        results.find(result => result._id === idregister)
                    )
                setOpenModal(true)
                break
            case 'downloadresult':
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_RESULT.find(
    ...
    const handleChangePagination = (newpage: number) => {
        ...
        if (filterMode) {
            !filterObjectSelected &&
                filterResults(filterSelected, 'fecha_resultado', newpage)
            !!filterObjectSelected &&
                filterResults(
