...
const GenerateGroupPaymentView: React.FunctionComponent<
    IGenerateGroupPaymentViewProps
> = ({ onCloseModal }) => {
    const { Option } = Select
    ...
    const [dataConfirmation, setDataConfirmation] =
        useState<IGroupConfirmPayment>(IGroupConfirmPaymentInitialization)
    ...
    const handleGroupConfirmPayments = async () => {
        ...
        const aux: IResponsePayment = await generateGroupPaymentsService(
            formData
        )
        if (!aux.err) {
    ...
    const handleFilterByDate = async (date: any) => {
        ...
        setTimeout(() => {
            ...
            let filteredByDate = payments?.filter(payment => {
                return moment(payment.fecha_facturacion, FORMAT_DATE).isBetween(
                    firstDate,
                    secondDate
                )
            })

            const aux = payments?.filter(
                element =>
                    element.fecha_facturacion ===
                        moment(firstDate).format(FORMAT_DATE) ||
                    element.fecha_facturacion ===
                        moment(secondDate).format(FORMAT_DATE)
            )
                ...
                <Row gutter={8}>
                    ...
                    <Col span={8}>
                        <Form.Item
                            ...
                            help={
                                !!dataConfirmation.sucursal ? '' : 'Seleccione'
                            }
                            label="Sucursal"
                        >
                            <Select
                ...
                <Row gutter={8}>
                    <Col span={12}>
                        <Form.Item
                            ...
                            help={
                                !!dataConfirmation.tipo_pago ? '' : 'Seleccione'
                            }
            ...
            <Form layout="vertical">
                ...
                >
                    ...
                    >
                        ...
                        />
                        <Form.Item label="Buscar por Fecha Facturación">
                            <RangePicker
