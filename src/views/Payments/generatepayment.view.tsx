...
import {
    ParcialPaymentInitialization,
    PaymentInitialization,
} from '../../initializations/payments.initialization'
...
const GeneratePaymentView: React.FunctionComponent<
    IGeneratePaymentViewProps
> = ({ onCloseModal, paymentSelected }) => {
    const { Option } = Select
    ...
    const [newDataParcialPayment, setNewDataParcialPayment] =
        useState<IPayment>(ParcialPaymentInitialization)
    ...
    const handleGenerateParcialPayment = async () => {
        ...
        const aux: IResponsePayment = await generateParcialPaymentService(
            paymentSelected?._id || '',
            formData
        )
        if (aux.err === null) {
    ...
    useEffect(() => {
        if (
            newDataParcialPayment.tipo_pago !== '' &&
        ...
        if (newDataParcialPayment.tipo_pago === 'Efectivo') {
            setNewDataParcialPayment({
                ...newDataParcialPayment,
                institucion_bancaria: 'No Aplica',
            })
        } else {
            setNewDataParcialPayment({
                ...newDataParcialPayment,
    ...
    return (
        <Spin spinning={loading} size="large" tip="Cargando...">
            ...
            )}
            {
                <Title
                    level={4}
                >{`${paymentSelected?.nombre_servicio} - ${paymentSelected?.codigo}`}</Title>
            }
            <Form layout="vertical">
                <Input.Group>
                    ...
                    <Row gutter={8}>
                        <Col span={12}>
                            <Form.Item label="Cliente principal">
                                <Input
                                    readOnly
                    ...
                    <Row gutter={8}>
                        ...
                        <Col span={12}>
                            <Form.Item
                                ...
                                validateStatus={
                                    newDataParcialPayment.institucion_bancaria !==
                                    ''
                                        ? 'success'
                                        : 'error'
                                }
                            ...
                            >
                                ...
                                'Efectivo' ? (
                                    ...
                                    <Input readOnly value="No Aplica" />
                                )}
                            </Form.Item>
                        </Col>
                    </Row>
                    <Row gutter={8}>
                                ...
                                <InputNumber
                                    ...
                                    onChange={e =>
                                        handleCalculateTotal(
                                            newDataParcialPayment.descuento,
                                            parseInt(e.toString())
                                        )
                                    }
                        ...
                        <Col span={8}>
                            <Form.Item
                                label="Total"
                                validateStatus={
                                    validAmmount ? 'success' : 'error'
                                }
                                help={
                                    validAmmount
                                        ? ''
                                        : 'Valor superior al valor pendiente'
                                }
                            >
                                <InputNumber
                                    style={{ width: '100%' }}
