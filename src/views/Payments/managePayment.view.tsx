...
import { Col, Input, Row, Card, Typography, Form, Table, Popover, Button, Spin } from 'antd';
...
const ManagePaymentView: React.FunctionComponent<IManagePaymentViewProps> = ({
  ...
  const handleDeletePayment = async (payment: IPayment) => {
    ...
    const aux: IResponsePayment = await deletePaymentService(paymentSelected?._id || '', payment);
    if (!aux.err) {
                  ...
                  render={(_, record: any) => {
                    return <>
                      <Popover
                        ...
                        trigger="click"
                        visible={(deleteConfirmation._id === record.id && deleteConfirmation.show === true) ? true : false}
                        onVisibleChange={() => setDeleteConfirmation({ _id: record.id, show: !deleteConfirmation.show })}
                      >
                        <Button
                          onClick={() => setDeleteConfirmation({ _id: record.id, show: true })}
