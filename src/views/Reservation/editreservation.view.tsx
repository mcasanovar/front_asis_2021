...
import { editReservationService, getOneReservationService } from '../../services/reservation.services';
...
const EditReservationView: React.FunctionComponent<IEditReservationViewProps> = ({
  ...
  const handleSaveRequest = async () => {
    ...
    const requestToInsert = MapReservation(newReservationData, reservationObs, assignProfessional?._id || '');
    formData.append("data", JSON.stringify(requestToInsert));
    const result: IResponseReservation = await editReservationService(newReservationData._id, formData);
    if (result.err === null) {
      ...
      <Input.Group>
        <Form layout='vertical'>
          <Row gutter={8}>
            ...
            <Col span={12}>
              <Form.Item
                validateStatus={newReservationData.id_GI_personalAsignado !== '' ? 'success' : 'error'}
                help={newReservationData.id_GI_personalAsignado !== '' ? '' : 'Debe seleccionar un profesional asignado'}
                label='Profesional asignado'
              >
                <Select
                  style={{ width: '100%' }}
                  onSelect={(e: SelectValue) => setNewReservationData({ ...newReservationData, id_GI_personalAsignado: e.toString() })}
                  id='error'
                  value={newReservationData.id_GI_personalAsignado}
                >
                  {workers.length > 0 && workers.map((worker, index) => (
                    <Option key={index} value={worker._id}>{`${worker.rut} - ${worker.razon_social}`}</Option>
                  ))}
                </Select>
              </Form.Item>
            </Col>
              ...
              >
                {newReservationData.hora_reserva !== '' &&
                  <TimePicker
                    ...
                    format="HH:mm"
                    clearIcon={false}
                    defaultValue={moment(newReservationData.hora_reserva, 'HH:mm')}
                    onChange={(e) => setNewReservationData({ ...newReservationData, hora_reserva: e?.format('HH:mm') || ''})}
                  />
                }
              </Form.Item>
            </Col>
              ...
              >
                {newReservationData.hora_reserva_fin !== '' &&
                  <TimePicker
                    ...
                    format="HH:mm"
                    clearIcon={false}
                    defaultValue={moment(newReservationData.hora_reserva_fin, 'HH:mm')}
                    onChange={(e) => setNewReservationData({ ...newReservationData, hora_reserva_fin: e?.format('HH:mm') || '' })}
                  />
                }
              </Form.Item>
            </Col>
          ...
          <Row>
            <Col span={24}>
              ...
              >
                <TextArea
                  rows={3}
                  onChange={(e) => setReservationObs(e.currentTarget.value)}
