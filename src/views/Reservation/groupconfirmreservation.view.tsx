...
import { Input, Row, Col, Select, DatePicker, Table, TimePicker, Form, Spin, Button } from "antd";
import { InboxOutlined } from "@ant-design/icons";

import AlertComponent from "../../component/Alert/Alert";

import { IDataReservationConfirmation, IResponseReservation, ReservationModel } from '../../models/reservation.models';
import { IDataReservationInitialization } from '../../initializations/reservation.initialization';
import { IAlertMessageContent } from '../../models/index.models';
import { confirmGroupReservationsService, getGroupReservationsService } from '../../services/reservation.services';
...
const GroupConfirmReservationView: React.FunctionComponent<IGroupConfirmReservationViewProps> = ({
  onCloseModal
}) => {
  const { Option } = Select;
  ...
  const [dataConfirmation, setDataConfirmation] = useState<IDataReservationConfirmation>(IDataReservationInitialization);
  ...
  const handleGroupConfirmReservations = async () => {
    ...
    const dataMapped = MapGroupReservationsToConfirm(dataConfirmation, selectedReservation);
    formData.append("data", JSON.stringify(dataMapped));
    const aux: IResponseReservation = await confirmGroupReservationsService(formData);
    if(!aux.err){
      ...
      <Form layout='vertical'>
        <Input.Group>
          <Row gutter={8}>
            <Col span={6}>
              <Form.Item
                ...
                validateStatus={dataConfirmation.fecha_reserva !== '' ? 'success' : 'error'}
                help={dataConfirmation.fecha_reserva !== '' ? '' : 'Seleccione fecha'}
              >
                <DatePicker
                  style={{ width: '100%' }}
                  format={FORMAT_DATE}
            ...
            <Col span={6}>
              <Form.Item
                ...
                validateStatus={dataConfirmation.hora_reserva !== '' ? 'success' : 'error'}
                help={dataConfirmation.hora_reserva !== '' ? '' : 'Seleccione hora'}
              >
                <TimePicker
                  style={{ width: '100%' }}
                  format="HH:mm"
            ...
            <Col span={6}>
              <Form.Item
                ...
                validateStatus={dataConfirmation.fecha_reserva_fin !== '' ? 'success' : 'error'}
                help={dataConfirmation.fecha_reserva_fin !== '' ? '' : 'Seleccione fecha'}
              >
                <DatePicker
                  style={{ width: '100%' }}
                  format={FORMAT_DATE}
            ...
            <Col span={6}>
              <Form.Item
                ...
                validateStatus={dataConfirmation.hora_reserva_fin !== '' ? 'success' : 'error'}
                help={dataConfirmation.hora_reserva_fin !== '' ? '' : 'Seleccione hora'}
              >
                <TimePicker
                  style={{ width: '100%' }}
                  format="HH:mm"
          ...
          <Row gutter={8}>
            <Col span={12}>
              <Form.Item
                validateStatus={dataConfirmation.id_GI_personalAsignado !== '' ? 'success' : 'error'}
                help={dataConfirmation.id_GI_personalAsignado !== '' ? '' : 'Debe seleccionar un profesional asignado'}
                label='Profesional asignado'
              >
                <Select
                  style={{ width: '100%' }}
                  onSelect={(e: SelectValue) => setDataConfirmation({ ...dataConfirmation, id_GI_personalAsignado: e.toString() })}
                  id='error'
                  value={dataConfirmation.id_GI_personalAsignado}
                >
            ...
            <Col span={6}>
              <Form.Item
                ...
                help={dataConfirmation.sucursal !== '' ? '' : 'Seleccione hora'}
              >
                <Select
                  style={{ width: '100%' }}
            ...
            <Col span={6}>
              <Form.Item
                ...
                validateStatus={dataConfirmation.reqEvaluacion !== '' ? 'success' : 'error'}
                help={dataConfirmation.reqEvaluacion !== '' ? '' : 'Seleccione hora'}
              >
                <Select
                  style={{ width: '100%' }}
                  onSelect={(e) => setDataConfirmation({ ...dataConfirmation, reqEvaluacion: e.toString() })}
                >
          ...
          <Row>
            <Col span={24}>
              ...
              >
                <TextArea
                  rows={3}
                  onChange={(e) => setDataConfirmation({ ...dataConfirmation, observacion: e.currentTarget.value })}
                />
              </Form.Item>
            </Col>
          </Row>
  ...
  const renderListReservations = () => {
    return (
      ...
      >
        ...
        <Column className='column-money' title="Fecha Reserva" dataIndex="fecha_reserva" key="fecha_reserva" />
        <Column className='column-money' title="Fecha Reserva Fin" dataIndex="fecha_reserva_fin" key="fecha_reserva_fin" />
  ...
  useEffect(() => {
    if (!dataConfirmation.fecha_reserva ||
