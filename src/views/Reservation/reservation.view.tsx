...
const ReservationView: React.FunctionComponent<IReservationViewProps> = ({
    authorized,
}) => {
    ...
    const [idSelectedReservation, setidSelectedReservation] =
        useState<string>('')
            ...
            case 'sendmail':
                idregister &&
                    setSelectedReservation(
                        reservations.find(
                            reservation => reservation._id === idregister
                        )
                    )
                setActualModal({
                    _id: id,
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_RESERVATION.find(
    ...
    const handleChangePagination = (newpage: number) => {
        ...
        if (filterMode) {
            !filterObjectSelected &&
                filterReservation(filterSelected, 'fecha_reserva', newpage)
            !!filterObjectSelected &&
                filterReservation(
    ...
    async function getReservations(pagenumber: number) {
        setLoading(true)
        const aux: IResponseAllReservations = await getAllReservationsService(
            pagenumber,
            N_PER_PAGE
        )
        if (!aux.err) {
        ...
        return setMessageAlert({
            message: 'No se ha podido cargar las reservas',
            type: 'error',
            show: true,
        })
    }
