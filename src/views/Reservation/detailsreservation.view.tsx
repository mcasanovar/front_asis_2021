...
> = ({ onCloseModal, _id = '' }) => {
    ...
    useEffect(() => {
        ...
        async function getOneReservations() {
            const aux: IResponseReservation = await getOneReservationService(
                _id
            )
            if (aux.err !== null) {
                        ...
                        <Col span={12}>
                            <Form.Item label="Profesional Asignado">
                                <Input
                                    readOnly
                                    value={
                                        assignProfessional?.razon_social &&
                                        `${assignProfessional.rut} - ${assignProfessional.razon_social}`
                                    }
                                />
                            </Form.Item>
                        </Col>
                    ...
                    <Row>
                        <Col span={24}>
                            <Form.Item label="Observacion">
                                {newReservationData.observacion.length > 0 &&
                                    newReservationData.observacion.map(
                                        (obs, index) => (
                                            <Input
                                                key={index}
                                                readOnly
