...
const InvoicesView: React.FunctionComponent<IInvoicesViewProps> = ({
    authorized,
}) => {
    ...
    const handleCLickActionTable = (
        ...
        switch (id) {
            case 'details':
                ...
                idregister &&
                    setInvoiceSelected(
                        invoices.find(invoice => invoice._id === idregister)
                    )
                setOpenModal(true)
                break
            case 'uploadoc':
    ...
    async function getInvoices(pagenumber: number) {
        setLoading(true)
        const aux: IResponseAllInvoices = await getAllInvoicesService(
            pagenumber,
            N_PER_PAGE
        )
        ...
        return setMessageAlert({
            message: 'No se ha podido cargar las facturaciones',
            type: 'error',
            show: true,
        })
    }
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_INVOICES.find(
