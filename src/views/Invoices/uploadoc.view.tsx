...
import { Input, Row, Col, Upload, DatePicker, TimePicker, Spin, Typography, Form, Table, Tag, Button } from "antd";
...
const UploadOCView: React.FunctionComponent<IUploadOCViewProps> = ({
  ...
  const [newDataInvoice, setNewDataInvoice] = useState<InvoicesModel>(InvoicesInitialization);
  ...
  const handleUploadOC = async () => {
    ...
    const result: IResponseInvoices = await uploadOCService(invoiceSelected?._id || '', formData);
    if (result.err === null) {
              ...
              <Input.Group>
                <Row gutter={8}>
                  <Col span={12}>
                    <Form.Item
                      label='Número OC'
                      validateStatus={newDataInvoice.nro_oc !== '' ? 'success' : 'error'}
                      help={newDataInvoice.nro_oc !== '' ? '' : 'Seleccionar'}
                    >
                      <Input
                        onChange={(e) => setNewDataInvoice({ ...newDataInvoice, nro_oc: e.currentTarget.value })}
                  ...
                  <Col span={6}>
                    <Form.Item
                      ...
                      validateStatus={newDataInvoice.fecha_oc !== '' ? 'success' : 'error'}
                      help={newDataInvoice.fecha_oc !== '' ? '' : 'Seleccionar'}
                    >
                      <DatePicker
                        style={{ width: '100%' }}
                        format={FORMAT_DATE}
                  ...
                  <Col span={6}>
                    <Form.Item
                      ...
                      validateStatus={newDataInvoice.hora_oc !== '' ? 'success' : 'error'}
                      help={newDataInvoice.hora_oc !== '' ? '' : 'Seleccionar'}
                    >
                      <TimePicker
                        format='HH:mm'
                        style={{ width: '100%' }}
