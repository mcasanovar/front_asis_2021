...
import { Input, Row, Col, Spin, Typography, Form, Button, Table, Tag } from "antd";
...
const DetailsEvaluationView: React.FunctionComponent<IDetailsEvaluationViewProps> = ({
  onCloseModal,
  _id = ''
}) => {
  ...
  const [newEvaluationData, setNewEvaluationData] = useState<EvaluationModel>(EvaluationInitialization);
  ...
  return (
    <Spin spinning={loading} size='large' tip='Cargando...'>
      ...
      {<Title level={4}>{`Evaluación ${newEvaluationData.codigo}`}</Title>}
      <Input.Group>
        <Form layout='vertical'>
          <Row gutter={8}>
            <Col span={8}>
              <Form.Item
                label='Código evaluación'
              >
                <Input
                  readOnly
          ...
          <Row gutter={8}>
            ...
            <Col span={8}>
              <Form.Item
                label='Fecha término evaluación'
              >
                <Input
                  readOnly
          ...
          <Row>
            <Col span={24} style={{
              ...
              <Table style={{ width: '100%' }} showHeader={true} dataSource={newEvaluationData.observaciones} pagination={false}>
