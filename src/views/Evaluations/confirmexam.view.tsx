...
const ConfirmExamView: React.FunctionComponent<IConfirmExamViewProps> = ({
    ...
    const [newDataEvaluation, setNewDataEvaluation] = useState<EvaluationModel>(
    ...
    //------------------------ USEEFECT
    useEffect(() => {
        if (newDataEvaluation.estado_archivo !== 'Cargado')
    ...
    return (
        <Spin spinning={loading} size="large" tip="Cargando...">
            ...
            )}
            {
                <Title
                    level={4}
                >{`${newDataEvaluation.nombre_servicio} - ${newDataEvaluation.codigo}`}</Title>
            }
            <Form layout="vertical">
                <Input.Group>
                    <Row gutter={8}>
                        ...
                        <Col span={8}>
                            <Form.Item label="Fecha evaluación">
                                <Input
                                    readOnly
                                    value={moment(
                                        newDataEvaluation.fecha_evaluacion,
                                        FORMAT_DATE
                                    ).format(FORMAT_DATE)}
                                />
                            </Form.Item>
                        ...
                        <Col span={8}>
                            <Form.Item label="Hora evaluación">
                                <Input
                                    readOnly
                                    value={
                                        newDataEvaluation.hora_inicio_evaluacion
                                    }
                                />
                            </Form.Item>
                        </Col>
                    ...
                    <Row gutter={8}>
                        ...
                        >
                            <Table
                                style={{ width: '100%' }}
                                showHeader={true}
                                dataSource={evaluationSelected?.observaciones}
                                pagination={false}
                            >
                                <Column
                                    width="20%"
