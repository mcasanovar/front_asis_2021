...
const EvaluationsView: React.FunctionComponent<IEvaluationsViewProps> = ({
    authorized,
}) => {
    ...
    const [evaluationSelected, setEvaluationSelected] =
        useState<EvaluationModel>()
            ...
            case 'uploadexam':
                ...
                idregister &&
                    setEvaluationSelected(
                        evaluations.find(
                            evaluation => evaluation._id === idregister
                        )
                    )
                setOpenModal(true)
                break
    ...
    async function getEvaluations(pagenumber: number) {
        setLoading(true)
        const aux: IResponseAllEvaluations = await getAllEvaluationsService(
            pagenumber,
            N_PER_PAGE
        )
        if (!aux.err) {
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_EVALUATION.find(
    ...
    const handleChangePagination = (newpage: number) => {
        ...
        if (filterMode) {
            !filterObjectSelected &&
                filterEvaluations(filterSelected, 'fecha_evaluacion', newpage)
            !!filterObjectSelected &&
                filterEvaluations(
    ...
    return (
        <div className="container-gi">
            ...
            <TableComponent
                ...
                onClickDelete={(id: string, _id?: string) =>
                    handleCLickActionTable(id, _id)
                }
            ...
            {ActualModal && (
                ...
                >
                    ...
                    {ActualModal._id === 'generateexam' &&
                        evaluationSelected?.nombre_servicio ===
