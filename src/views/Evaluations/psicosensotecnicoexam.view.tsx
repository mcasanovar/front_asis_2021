...
import { Collapse, Input, Row, Col, Select, Checkbox, Typography, Popover, Button, Spin, Form } from "antd";
import { QuestionCircleOutlined, PercentageOutlined } from "@ant-design/icons";

import { IAlertMessageContent, IExamPsicoExtras } from '../../models/index.models';
...
import { EvaluationModel, ExamPsicosensotecnicoModel, IResponseEvaluation } from '../../models/evaluations.models';
...
const PsicosensotecnicoExamView: React.FunctionComponent<IPsicosensotecnicoExamViewProps> = ({
  onCloseModal,
  evaluationSelected
}) => {
  ...
  const [newExamData, setNewExamData] = useState<ExamPsicosensotecnicoModel>(ExamPsicosensotecnicoInitialization);
  const [ExamExtra, setExamExtra] = useState<IExamPsicoExtras[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [messageAlert, setMessageAlert] = useState<IAlertMessageContent>({ message: '', type: 'success', show: false });

  const options: IExamPsicoExtras[] = [
    { label: 'Anticipación', value: 'anticipacion', key: '4', name_exam: 'is_anticipacion' },
    { label: 'Somnolencia', value: 'somnolencia', key: '5', name_exam: 'is_somnolencia' },
    { label: 'Psicológico', value: 'psicologico', key: '6', name_exam: 'is_psicologico' },
    { label: 'Monotonia', value: 'Monotonia', key: '7', name_exam: "is_monotonia" },
    { label: 'Reacciones multiples', value: 'reactiones-multiple', key: '8', name_exam: 'is_reacciones_multiples' },
    ...
    return (
      <Form layout='vertical'>
        <Input.Group>
          <Row>
            <Col span={24}>
              <Title level={5} style={{ textAlign: 'center' }}>Información personal</Title>
            </Col>
          </Row>
          <Row gutter={8}>
            <Col span={8}>
              <Form.Item
                validateStatus={newExamData.licencia_a_acreditar.length > 0 ? 'success' : 'error'}
                help={newExamData.licencia_a_acreditar.length > 0 ? '' : 'seleccionar'}
                label='Licencia a acredita clase'
              >
                <Select
                  style={{ width: '100%' }}
                  onChange={(e: any) => setNewExamData({ ...newExamData, licencia_a_acreditar: e })}
                  mode='multiple'
            ...
            <Col span={8}>
              <Form.Item
                validateStatus={newExamData.resultado !== '' ? 'success' : 'error'}
                help={newExamData.resultado !== '' ? '' : 'seleccionar'}
                label='Resultado'
              >
                <Select
                  style={{ width: '100%' }}
    ...
    return (
      <Form layout='vertical'>
        <Input.Group>
          ...
          <Row>
            <Col span={24}>
              <Paragraph style={{ fontWeight: 'bold' }}>Agudeza visual.</Paragraph>
            </Col>
          </Row>
          <br />
          <Row gutter={8}>
  ...
  const handleGenerateExam = async () => {
    const examToInsert = MapPsicoToCreateExam(evaluationSelected, newExamData, ExamExtra);
    setLoading(true)
    const aux: IResponseEvaluation = await createExamPsicoService(examToInsert);
    if (aux.err === null) {
  ...
  //---------------------------------------USEEFECT
  useEffect(() => {
    if (newExamData.licencia_a_acreditar.length > 0
      ...
      <>
        ...
        <Form layout='vertical'>
          <Input.Group>
            <Row gutter={8}>
              <Col span={4}>
                <Form.Item
                  ...
                  validateStatus={newExamData.examen_psicotecnico !== '' ? 'success' : 'error'}
                  help={newExamData.examen_psicotecnico !== '' ? '' : 'seleccionar'}
                  style={{ width: '100%', height: '100%' }}
                >
                  <Select
                    style={{ width: '100%' }}
              ...
              <Col span={4}>
                <Form.Item
                  ...
                  validateStatus={newExamData.examen_sensometrico !== '' ? 'success' : 'error'}
                  help={newExamData.examen_sensometrico !== '' ? '' : 'seleccionar'}
                  style={{ width: '100%', height: '100%' }}
                >
                  <Select
                    style={{ width: '100%' }}
            ...
            {ExamExtra.some(exam => exam.key === '8' || exam.key === '9') && (
              <Row gutter={8}>
            ...
            <Row gutter={8}>
              <Col span={24}>
                <Form.Item
                  ...
                  validateStatus={newExamData.conclusion_recomendacion !== '' ? 'success' : 'error'}
                  help={newExamData.conclusion_recomendacion !== '' ? '' : 'escribir conclusión'}
                  style={{ width: '100%', height: '100%' }}
                >
                  <TextArea
                    rows={3}
            ...
            <Row>
              <Col>
                <Form.Item
                  ...
                  validateStatus={newExamData.meses_vigencia !== 0 ? 'success' : 'error'}
                  help={newExamData.meses_vigencia !== 0 ? '' : 'seleccionar'}
                  style={{ width: '60%', height: '100%' }}
                >
                  <Input
                    onChange={(e) => setNewExamData({ ...newExamData, meses_vigencia: parseInt(e.currentTarget.value) })}
                    type='number'
                    id='error_6'
                  />
                </Form.Item>
