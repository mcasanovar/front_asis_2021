...
const CreateGiView: FunctionComponent<ICreateGiViewProps> = ({
    ...
    const [organizationsBelonging, setOrganizationsBelonging] = useState<
        GiModel[]
    >([])
    const [organizationBelongingSelected, setOrganizationBelongingSelected] =
        useState<GiModel>(GiInitializationData)
    ...
    const handlePrimaryActivity = (e: SelectValue) => {
        ...
        if (newGiData.actividad_secundaria.codigo === e) {
            setMessageAlert({
                message:
                    'No puede seleccionar la misma actividad primaria y secundaria',
                type: 'error',
                show: true,
            })
        }
    ...
    const handleSetBirthDate = (e: Moment) => {
        ...
        setNewGiData({
            ...newGiData,
            fecha_inic_mac: e.format(FORMAT_DATE),
            edad_gi: age,
        })
    }
    ...
    const handleFaenasToAdd = (e: any) => {
        const aux: IFaena[] = e.map((faena: string) => {
            return { name: faena.toString() }
        })
        setFaenasToAdd(aux)
    }
    ...
    const handleAddContractFaenas = () => {
        ...
        setNewGiData({
            ...newGiData,
            contrato_faenas: [...newGiData.contrato_faenas, ...[aux]],
        })
    ...
    const editContractFaenas = () => {
        ...
        const aux: IContract[] = newGiData.contrato_faenas.map(
            (contract, index) => {
    ...
    const handleDeleteContractFaena = (index: number) => {
        ...
        const aux: IContract[] = newGiData.contrato_faenas.filter(
    ...
    const handleCalculateLicenseState = (e: Moment) => {
        ...
        setNewGiData({
            ...newGiData,
            fecha_venc_licencia: e.format(FORMAT_DATE),
            estado_licencia: aux + 1 <= 0 ? 'No Vigente' : 'Vigente',
        })
    }
    ...
    const handleInsertGI = async () => {
        ...
        if (result.err === null) {
            ...
            if (result.err === 99) setLoading(false)
    ...
    const handleSaveGI = async () => {
        ...
        file !== null && formData.append('archivo', file)
        const result: IResponseGI = await editOneGIService(
            newGiData._id,
            formData
        )
        if (result.err === null) {
    ...
    async function getOneGI() {
        ...
        const selected = organizationsBelonging.find(
            org => org._id === gi.id_GI_org_perteneciente
        )
    ...
    useEffect(() => {
        if (
            ...
            !!newGiData.nro_contrato &&
            type === 'edit'
        ) {
            const aux = organizationBelongingSelected.contrato_faenas.find(
                (contract: IContract) =>
                    contract.nro_contrato === newGiData.nro_contrato
            )
            if (!aux) return
            setFaenasSelected(aux.faenas)
        }
    ...
    useEffect(() => {
        if (newGiData.categoria === 'Empresa/Organizacion') {
            setNewGiData({
                ...newGiData,
                categoria_empresa: newGiData.categoria_empresa,
    ...
    useEffect(() => {
        if (newGiData.grupo_interes === 'Empleados')
            return setNewGiData({
                ...newGiData,
                categoria_cliente: 'No Aplica',
            })
        return setNewGiData({
            ...newGiData,
            categoria_cliente: newGiData.categoria_cliente || 'Ocacional',
        })
    }, [newGiData.grupo_interes])

    useEffect(() => {
        if (newGiData.credito === 'No')
            return setNewGiData({ ...newGiData, dias_credito: 0 })
    }, [newGiData.credito])
    ...
    useEffect(() => {
        if (
            ...
            if (newGiData.categoria === 'Persona Natural') {
                if (
                    newGiData.id_GI_org_perteneciente !== '' &&
                    ...
                    <Row gutter={8}>
                        <Col span={6}>
                            <Form.Item
                                ...
                                help={
                                    isRutOk ? '' : 'Formato de rut incorrecto'
                                }
                                label="Rut"
                            >
                                <Input
                            ...
                            <Form.Item label="Actividad principal">
                                ...
                                >
                                    {ACTIVITY_DATA.map((activity, index) => (
                                        ...
                                        >{`${activity.codigo} - ${activity.actividad}`}</Option>
                                    ))}
                                </Select>
                            </Form.Item>
                        </Col>
                    ...
                    <Row gutter={8}>
                                ...
                                {(newGiData.categoria ===
                                    ...
                                    <Select
                                        ...
                                        onChange={e =>
                                            setNewGiData({
                                                ...newGiData,
                                                categoria_empresa: e.toString(),
                                            })
                                        }
                                        value={newGiData.categoria_empresa}
                                    >
                                        {COMPANY_CATEGORY.map(
                                            (category, index) => (
                                                <Option
                                                    key={index}
                                                    value={category}
                        ...
                        <Col span={6}>
                            <Form.Item label="Categoria cliente">
                                {newGiData.grupo_interes === 'Empleados' ? (
                                    <Input
                ...
                <Form layout="vertical">
                    <Row gutter={8}>
                        ...
                        <Col span={6}>
                            <Form.Item
                                validateStatus={
                                    isPrimaryEmailOk ? 'success' : 'error'
                                }
                                help={
                                    isPrimaryEmailOk
                                        ? ''
                        ...
                        <Col span={6}>
                            <Form.Item
                                validateStatus={
                                    isSecondaryEmailOk ? 'success' : 'error'
                                }
                                help={
                                    isSecondaryEmailOk
                                        ? ''
                            ...
                            >
                                <Input
                                    ...
                                    type="email"
                                    onBlur={e =>
                                        handleValidateEmail(e, 'secondary')
                                    }
                                    onChange={e =>
                                        setNewGiData({
                                            ...newGiData,
                            ...
                            <Row gutter={8}>
                                <Col span={6}>
                                    <Form.Item label="Nro. contrato">
                                        {!!organizationBelongingSelected &&
                                        organizationBelongingSelected
                                            ?.contrato_faenas.length ? (
                                            <Select
                                                style={{ width: '100%' }}
                                                onChange={e =>
                                                    handleSetContractNumber(e)
                                                }
                                                value={newGiData.nro_contrato}
                                            >
                                                {organizationBelongingSelected.contrato_faenas.map(
                                                    (contract, index) => (
                                                        <Option
                                                            key={index}
                                                            value={
