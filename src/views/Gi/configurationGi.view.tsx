...
import { ConfigurationGIModel, GiModel, IResponseEmployees } from '../../models/gi.models';
...
const ConfigurationGi: React.FunctionComponent<IConfigurationGiProps> = ({
  ...
  const [newDataConfiguracionGI, setnewDataConfiguracionGI] = useState<ConfigurationGIModel>({ ...ConfigurationGIInitialization, rol: giSelected?.rol || '' });
  ...
  const handleSaveConfiguration = async () => {
    setLoading(true);
    const aux: IResponseEmployees = await editPasswordService(giSelected?._id || '', newDataConfiguracionGI);
    if (aux.err === null) {
  ...
  const handleValidPassword = (confirmpass: string) => {
    setisValidPassword(newDataConfiguracionGI.new_password === confirmpass ? true : false)
  };

  //--------------------------------------USEEFECT
  useEffect(() => {
    if (!newDataConfiguracionGI.isEditPassword) return setDisabledConfirm(false);
        ...
        <Row gutter={8}>
          <Col span='12'>
            ...
            >
              <Select
                style={{ width: '100%' }}
                onSelect={(e) => setnewDataConfiguracionGI({ ...newDataConfiguracionGI, rol: e.toString() })}
          ...
          <Col span='12'>
            ...
            >
              <Select
                style={{ width: '100%' }}
                onSelect={(e) => setnewDataConfiguracionGI({ ...newDataConfiguracionGI, isEditPassword: e.toString() === 'Si' ? true : false })}
              >
          ...
          <Row gutter={8}>
            <Col span={12}>
              ...
              >
                <Input.Password
                  maxLength={12}
                  minLength={6}
                  onChange={(e) => setnewDataConfiguracionGI({ ...newDataConfiguracionGI, new_password: e.currentTarget.value })}
                  prefix={<LockOutlined />}
                />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
                ...
                help={isValidPassword ? '' : 'Contraseñas no coinciden'}
                label='Repetir contraseña'
              >
                <Input.Password
                  maxLength={12}
                  minLength={6}
                  onChange={(e) => handleValidPassword(e.currentTarget.value)}
                  prefix={<LockOutlined />}
                />
              </Form.Item>
            </Col>
