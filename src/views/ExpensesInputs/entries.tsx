...
import { Spin, Typography, Form, Collapse, Input, Row, Col, Button, Select, Tag, InputNumber, Table, Popover } from 'antd';
import { IAlertMessageContent, ITableDeleteObject } from '../../models/index.models';
...
const EntriesView: React.FunctionComponent<IEntriesViewProps> = ({
  ...
  const [newDataEntry, setNewDataEntry] = useState<IEntries>(EntriesInitialization);
  const [entries, setEntries] = useState<IEntries[]>(expenseSelected?.entradas || []);
  const [disabledConfirm, setDisabledConfirm] = useState<boolean>(true);
  const [disabledAdd, setDisabledAdd] = useState<boolean>(true);
  const [reloadTable, setReloadTable] = useState<boolean>(true);
  const [subCategoryThree, setSubCategoryThree] = useState<{ codigo: string, subcategoria_tres: string }>({ codigo: '', subcategoria_tres: '' });
  ...
  const handleGetSubcategoryThree = () => {
    const category = CATEGORIES_EXPENSES.find((category) => category.categoria_general === expenseSelected?.categoria_general);
    if (!category) return [];

    const subCagegoryOne = category.subcategoria_uno.find((subone) => subone.subcategoria_uno === expenseSelected?.subcategoria_uno);
    if (!subCagegoryOne) return [];

    const subCategoryTwo = subCagegoryOne.subcategoria_dos.find((subtwo) => subtwo.subcategoria_dos === expenseSelected?.subcategoria_dos);
  ...
  const handleAssignSubcategoryThree = (code: string) => {
    ...
    const aux = subCategoryTwo.sub_cat_tres.find((subthree) => subthree.codigo === code);
    if (!aux) return;
  ...
  const handleCalculateTotalEntries = () => {
    const aux = entries.reduce((acc, current) => acc + current.costo_total, 0);
    return aux || 0;
  };

  const handleCalculatePendingAmmountEntries = () => {
    const payed = entries.reduce((acc, current) => acc + current.costo_total, 0) || 0;
  ...
  const handleInsertEntries = async () => {
    setLoading(true);
    const aux: IResponseExpenses = await insertEntriesService(expenseSelected?._id || '', entries);
    if (aux.err === null) {
      setMessageAlert({ message: 'Información guardada correctamente', type: 'success', show: true });
      setLoading(false);
      return
    }
      ...
      <Input.Group>
        ...
        <Row gutter={8}>
          <Col span={6}>
            ...
            >
              <Select
                style={{ width: '100%' }}
                onSelect={(e) => handleAssignSubcategoryThree(e.toString())}
                value={`${subCategoryThree?.codigo} - ${subCategoryThree?.subcategoria_tres}`}
              >
                {handleGetSubcategoryThree().map((subthree, index) => (
                  <Option key={index} value={subthree.codigo}>{`${subthree.codigo} - ${subthree.subcategoria_tres}`}</Option>
                ))}
              </Select>
            </Form.Item>
          </Col>
        ...
        <Row gutter={8}>
          <Col span={4}>
            <Form.Item
              label='Cantidad'
              validateStatus={newDataEntry.cantidad !== 0 ? 'success' : 'error'}
              help={newDataEntry.cantidad !== 0 ? '' : 'ingrese valor'}
            >
              <Input
                type='number'
                style={{ width: '100%' }}
          ...
          <Col span={4}>
            <Form.Item
              ...
              validateStatus={newDataEntry.costo_unitario !== 0 ? 'success' : 'error'}
              help={newDataEntry.costo_unitario !== 0 ? '' : 'ingrese valor'}
            >
          ...
          <Col span={5}>
            <Form.Item
              validateStatus={newDataEntry.costo_total <= handleCalculatePendingAmmountEntries() ? 'success' : 'error'}
              help={newDataEntry.costo_total <= handleCalculatePendingAmmountEntries() ? '' : 'Valor excede el monto pendiente'}
              label='Costo total'
            >
              <InputNumber
                style={{ width: '100%' }}
        ...
        <Row gutter={8}>
          <Col span={24}>
            ...
            >
              <TextArea
                rows={2}
                onChange={(e) => setNewDataEntry({ ...newDataEntry, detalle: e.currentTarget.value })}
        ...
        >
          <Button
            ...
            style={!disabledAdd ?
              { backgroundColor: 'green', borderColor: 'green', color: 'white', width: '10%', height: '100%' } :
              { backgroundColor: 'grey', borderColor: 'grey', color: 'white', width: '10%', height: '100%' }}
          >
            Agregar
          </Button>
        </Row>
      ...
      <Form layout='vertical'>
        <Row gutter={8}>
          <Col span={6}>
            ...
            >
              {`Gasto total : $ ${MilesFormat(expenseSelected?.total || 0)}`}
            </Tag>
          </Col>
          <Col span={6}>
            <Tag
            ...
            >
              {`Gasto pagado : $ ${MilesFormat(handleCalculateTotalEntries())}`}
            </Tag>
          </Col>
          <Col span={6}>
            <Tag
            ...
            >
              {`Gasto pendiente : $ ${MilesFormat(handleCalculatePendingAmmountEntries())}`}
            </Tag>
          </Col>
        </Row>
        <br />
