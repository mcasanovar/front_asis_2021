...
const CalendarScreen: React.FunctionComponent<ICalendarScreenProps> = ({
    ...
    const handleDataRenderDates = (value: Moment) => {
        const auxRequests = requests.filter(
            request =>
                moment(request.fecha_solicitud, FORMAT_DATE).format(
                    FORMAT_DATE
                ) === value.format(FORMAT_DATE)
        )
        const auxReservations = reservations.filter(
            reservation =>
                reservation.fecha_reserva === value.format(FORMAT_DATE)
        )
        return (
            <ul className="events">
                {!!auxRequests.length &&
                    auxRequests.map(item => (
                        <li key={item._id}>
                            <Tag
                                color={
                ...
                {!!auxReservations.length &&
                    auxReservations.map(item => (
                        <li key={item._id}>
                            <Tag
                                color={
                                    item.estado === 'Ingresado'
    ...
    const handleDataRenderMonths = (value: Moment) => {
        const auxRequests = requests.filter(
            request =>
                moment(request.fecha_solicitud, FORMAT_DATE).format('MMMM') ===
                value.format('MMMM')
        )
        const auxReservations = reservations.filter(
            reservation =>
                moment(reservation.fecha_reserva, FORMAT_DATE).format(
                    'MMMM'
                ) === value.format('MMMM')
        )
        return (
    ...
    const getRequests = async (
        ...
        setLoading(true)
        const aux: IResponseAllRequests = await getRequestsByDateService(
            month,
            year
        )
        if (!aux.err) {
    ...
    const getReservations = async (
        ...
        const aux: IResponseAllReservations =
            await getReservationsByDateService(month, year)
        if (!aux.err) {
