...
import { Collapse, Input, Form, Col, Row, Select, Tag, Table, Typography } from "antd";
...
const DetailsCalendarScreen: React.FunctionComponent<IDetailsCalendarScreenProps> = ({
  requests,
  reservations,
  currentDate,
  workers
  ...
  const [reservationSelected, setReservationSelected] = useState<ReservationModel>();
  const [obsRequestSelected, setObsRequestSelected] = useState<any>([]);
  const [workerSelectedRequest, setWorkerSelectedRequest] = useState<GiModel>();
  const [workerSelectedReservation, setWorkerSelectedReservation] = useState<GiModel>();

  const handleSelectRequest = (id: string) => {
    const selected = requests.find((request) => request._id === id);
    if(!!selected){
      const auxWorker = workers.find((worker) => worker._id === selected.id_GI_PersonalAsignado)
  ...
  const handleSelectReservation = (id: string) => {
    const selected: ReservationModel | undefined = reservations.find((reservation) => reservation._id === id);
    if(!!selected){
      const auxWorker = workers.find((worker) => worker._id === selected.id_GI_personalAsignado)
        ...
        <Form layout='vertical'>
            ...
            <Col span='18'>
              ...
              >
                <Select
                  ...
                  onSelect={(e) => handleSelectRequest(e.toString())}
                >
              ...
              <>
                ...
                <Col span='4'>
                  <Form.Item
                    label='.'
                  >
                    <Tag color={requestSelected.estado === 'Ingresado' ? '#15B8D5' : 'green'}>{requestSelected.estado}</Tag>
                  </Form.Item>
                </Col>
              </>
            }
          ...
          <Row>
            <Col span={24} style={{
              ...
              <Table style={{ width: '100%' }} showHeader={true} dataSource={!!obsRequestSelected && obsRequestSelected} pagination={false}>
      ...
      <Input.Group>
        <Form layout='vertical'>
            ...
            <Col span='18'>
              ...
              >
                <Select
                  ...
                  onSelect={(e) => handleSelectReservation(e.toString())}
                >
                  {reservations.map((reservation, index) => (
                    <Option key={index} value={reservation._id}>
              ...
              <>
                ...
                <Col span='4'>
                  <Form.Item
                    label='.'
                  >
                    <Tag color={reservationSelected.estado === 'Ingresado' ? '#1073B5' : '#21B831'}>{reservationSelected.estado}</Tag>
                  </Form.Item>
                </Col>
              </>
            }
          ...
          <Row gutter={8}>
            ...
            <Col span='4'>
              <Form.Item
                label='Fecha Reserva Término'
              >
                <Input
                  readOnly
                  value={reservationSelected?.fecha_reserva_fin}
                />
              </Form.Item>
            </Col>
            <Col span='4'>
              <Form.Item
                label='Hora Reserva Término'
              >
                <Input
                  readOnly
                  value={reservationSelected?.hora_reserva_fin}
                />
              </Form.Item>
            </Col>
            <Col span='8'>
              ...
              >
                <Input
                  readOnly
                  value={workerSelectedReservation?.razon_social}
                />
              </Form.Item>
            </Col>
          </Row>
          <Row>
            <Col span={24} style={{
              ...
              <Table style={{ width: '100%' }} showHeader={true} dataSource={reservationSelected?.observacion} pagination={false}>
                <Column width='20%' className='column-money' title="Fecha" dataIndex="fecha" key="fecha" />
                <Column width='60%' className='column-money' title="Observación" dataIndex="obs" key="obs" />
              </Table>
            </Col>
  ...
  return (
    <>
      {<Title level={4}>{currentDate.format('DD [de] MMMM YYYY').toUpperCase()}</Title>}
