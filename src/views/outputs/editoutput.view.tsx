...
import { editOutputService, getExpenseByCategory, insertOutputService } from '../../services';
...
const EditOuputView: React.FunctionComponent<IEditOuputViewProps> = ({
  ...
  const [newDataOutput, setNewDataOutput] = useState<OutputModel>(outputSelected || OutputsInitialization);
  ...
  const handleEditOutput = async () => {
    ...
    const aux: IResponseOutputs = await editOutputService(newDataOutput._id || '', newDataOutput);
    if (aux.err === null) {
      ...
      <>
        <Row gutter={8}>
          <Col span={4}>
            <Form.Item
              label='Cantidad'
              validateStatus={(newDataOutput.cantidad !== 0) ? 'success' : 'error'}
              help={(newDataOutput.cantidad !== 0) ? '' : cantMessage}
            >
              <Input
                type='number'
          ...
          <Col span={4}>
            <Form.Item
              ...
              validateStatus={newDataOutput.costo_unitario ? 'success' : 'error'}
              help={newDataOutput.costo_unitario ? '' : 'Seleccione'}
            >
              <InputNumber
