...
const EditOuputView: React.FunctionComponent<IEditOuputViewProps> = ({
    outputSelected,
    onCloseModal,
}) => {
    ...
    const [subcategorythreemessage, setSubcategorythreemessage] =
        useState<string>('')
    ...
    const [newDataOutput, setNewDataOutput] = useState<OutputModel>(
        outputSelected || OutputsInitialization
    )
    ...
    const handleSelectedSubcagoryOne = (index: number) => {
        ...
        setNewDataOutput({
            ...newDataOutput,
            subcategoria_uno:
                CATEGORIES_EXPENSES[indexsubone].subcategoria_uno[index]
                    .subcategoria_uno,
            subcategoria_dos: '',
            subcategoria_tres: '',
        })
    ...
    const handleSelectedSubcagoryTwo = (index: number) => {
        ...
        const aux =
            CATEGORIES_EXPENSES[indexsubone].subcategoria_uno[indexsubtwo]
                .subcategoria_dos[index]
    ...
    const handleSelectedSubcategoryThree = async (index: number) => {
        ...
        if (aux.err === null) {
            const auxSubcategory =
                CATEGORIES_EXPENSES[indexsubone].subcategoria_uno[indexsubtwo]
                    .subcategoria_dos[indexsubthree].sub_cat_tres[index]
            setNewDataOutput({
                ...newDataOutput,
            ...
            const existsEntry = expense.entradas.find(
                (entry: IEntries) =>
                    entry.codigo_categoria_tres === auxSubcategory.codigo
            )
            if (!existsEntry) {
                setSubcategorythreemessage(
                    'No se encuentra entradas para esta subcategoria'
                )
                setLoading(false)
                return
            }
    ...
    const handleCalculateTotalCost = (cant: number) => {
        const totalCost = cant * (newDataOutput.costo_unitario || 0)
    ...
    const handleEditOutput = async () => {
        ...
        const aux: IResponseOutputs = await editOutputService(
            newDataOutput._id || '',
            newDataOutput
        )
        if (aux.err === null) {
                ...
                <Row gutter={8}>
                        ...
                        <Form.Item label="Nro. Documento">
                            <Input
                                onChange={e =>
                                    setNewDataOutput({
                                        ...newDataOutput,
                                        nro_documento: e.currentTarget.value,
                                    })
                                }
                    ...
                    <Col span={6}>
                        <Form.Item label="Usuario sistema">
                            <Input readOnly value={newDataOutput.usuario} />
                        </Form.Item>
                    </Col>
                </Row>
                <Row gutter={8}>
                        ...
                        >
                            ...
                            >
                                {indexsubone >= 0 &&
                                    CATEGORIES_EXPENSES[
                                        indexsubone
                                    ].subcategoria_uno.map((sub1, index) => (
                                        <Option key={index} value={index}>
                                            {sub1.subcategoria_uno}
                                        </Option>
                                    ))}
                            </Select>
                        </Form.Item>
                            ...
                            >
                                {indexsubtwo >= 0 &&
                                    CATEGORIES_EXPENSES[
                                        ...
                                        indexsubtwo
                                    ].subcategoria_dos.map((sub2, index) => (
                                        <Option key={index} value={index}>
                                            {sub2.subcategoria_dos}
                                        </Option>
                                    ))}
                            </Select>
                        </Form.Item>
                                ...
                                >
                                    {indexsubthree >= 0 &&
                                        CATEGORIES_EXPENSES[
                                            ...
                                            indexsubthree
                                        ].sub_cat_tres.map((sub3, index) => (
                                            <Option
                                                key={index}
                                                value={index}
                                            >{`${sub3.codigo} - ${sub3.subcategoria_tres}`}</Option>
                                        ))}
                                </Select>
                            </Form.Item>
                        </Col>
                        ...
                        <Form.Item label="Motivo salida">
                            <TextArea
                                ...
                                onChange={e =>
                                    setNewDataOutput({
                                        ...newDataOutput,
                                        motivo_salida: e.currentTarget.value,
                                    })
                                }
                ...
                <Row gutter={8}>
                    <Col span={4}>
                        <Form.Item
                            ...
                            help={
                                newDataOutput.cantidad !== 0 ? '' : cantMessage
                            }
                        >
                            <Input
                                type="number"
                    ...
                    <Col span={4}>
                        <Form.Item
                            ...
                            validateStatus={
                                newDataOutput.costo_unitario
                                    ? 'success'
                                    : 'error'
                            }
                            help={
                                newDataOutput.costo_unitario ? '' : 'Seleccione'
                            }
                        >
                            <InputNumber
                                formatter={value =>
                                ...
                                defaultValue={newDataOutput.costo_unitario || 0}
                                style={{ width: '100%' }}
                                readOnly
                                value={newDataOutput.costo_unitario}
                            />
                        </Form.Item>
                    </Col>
                    <Col span={4}>
