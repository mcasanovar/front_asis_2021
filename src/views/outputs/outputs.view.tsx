...
const OutputsView: React.FunctionComponent<IOutputsViewProps> = ({
    authorized,
}) => {
    ...
    const handleCLickActionTable = (
        ...
        switch (id) {
            case 'details':
                ...
                idregister &&
                    setOutputSelected(
                        outputs.find(output => output._id === idregister)
                    )
                setOpenModal(true)
                break
            case 'edit':
    ...
    const handleTransformValues = (outputs: OutputModel[]) => {
        const aux = outputs.map(output => {
            return {
                ...output,
                costo_unitario_string: `$ ${MilesFormat(
                    output.costo_unitario
                )}`,
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_OUTPUT.find(
    ...
    async function getOutputs(pagenumber: number) {
        const aux: IResponseAllOutputs = await getAllOutputsService(
            pagenumber,
            N_PER_PAGE
        )
        if (!aux.err) {
        ...
        return setMessageAlert({
            message: 'No se ha podido cargar los salidas',
            type: 'error',
            show: true,
        })
    }
