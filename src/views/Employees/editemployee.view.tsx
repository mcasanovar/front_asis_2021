...
import { Col, Input, Row, Select, DatePicker, Button, Form, InputNumber, Spin } from "antd";
import { DollarOutlined, InboxOutlined } from "@ant-design/icons";
import { GiModel, IResponseEmployees } from '../../models/gi.models';
import { AFP_LIST, CONTRACT_TYPES, FORMAT_DATE, ISAPRE_LIST, SECURITY_WORK } from '../../constants/var';
...
const EditEmployeeView: React.FunctionComponent<IEditEmployeeViewProps> = ({
  ...
  const handleEditEmployee = async () => {
    ...
    const aux: IResponseEmployees = await editEmployeeService(employeesSelected?._id || '', employeeMapped);
    if (aux.err === null) {
  ...
  useEffect(() => {
    if (newDataEmployee.fecha_fin_contrato !== '') {
      setNewDataEmployee({
        ...newDataEmployee,
        estado_contrato: handleComparateDates(moment(newDataEmployee.fecha_fin_contrato))
      });
    }
      ...
      <Form layout='vertical'>
        <Input.Group>
          ...
          <Row gutter={8}>
            ...
            {newDataEmployee.tipo_contrato !== 'Contrato Indefinido' &&
              <Col span={5}>
                <Form.Item
          ...
          <Row gutter={8}>
            ...
            <Col span={9}>
              ...
              >
                <Select
                  style={{ width: '100%' }}
                  onSelect={(e) => setNewDataEmployee({ ...newDataEmployee, afp: e.toString() })}
                  value={newDataEmployee.afp}
                >
                  {AFP_LIST.map((afp, index) => (
                    <Option key={index} value={afp}>{afp}</Option>
                  ))}
                </Select>
              </Form.Item>
            </Col>
            <Col span={9}>
              ...
              >
                <Select
                  style={{ width: '100%' }}
                  onSelect={(e) => setNewDataEmployee({ ...newDataEmployee, isapre: e.toString() })}
                  value={newDataEmployee.isapre}
                >
                  {ISAPRE_LIST.map((isapre, index) => (
                    <Option key={index} value={isapre}>{isapre}</Option>
                  ))}
                </Select>
              </Form.Item>
            </Col>
          ...
          <Row gutter={8}>
            <Col span={8}>
              ...
              >
                <Select
                  style={{ width: '100%' }}
                  onSelect={(e) => setNewDataEmployee({ ...newDataEmployee, seguridad_laboral: e.toString() })}
            ...
            <Col span={16}>
              ...
              >
                <Input
                  onChange={(e) => setNewDataEmployee({ ...newDataEmployee, comentarios: e.currentTarget.value })}
