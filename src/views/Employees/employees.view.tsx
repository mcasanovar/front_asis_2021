...
const Employees: React.FunctionComponent<IEmployeesProps> = ({
    authorized,
}) => {
            ...
            case 'edit':
                ...
                idregister &&
                    setEmployeeSelected(
                        employees.find(
                            employee =>
                                employee._id === idregister &&
                                employee.activo_inactivo === true
                        )
                    )
                setOpenModal(true)
                break
    ...
    const handleTransformPrice = (employees: GiModel[]) => {
        const aux = employees.map(employee => {
            return {
                ...employee,
                sueldo_bruto_string: `$ ${MilesFormat(
                    employee.sueldo_bruto || 0
                )}`,
            }
        })
        return aux
    ...
    const handleClickSearch = async () => {
        setLoading(true)
        const headfilter = FILTERS_EMPLOYEES.find(
    ...
    async function getEmployees(pagenumber: number) {
        setLoading(true)
        const aux: IReponseAllEmployees = await getAllEmployeesService(
            pagenumber,
            N_PER_PAGE
        )
        if (!aux.err) {
                ...
                >
                    {ActualModal._id === 'edit' && (
                        <EditEmployeeView
                            ...
                            employeesSelected={
                                employeeSelected || { ...GiInitializationData }
                            }
                        />
                    )}
