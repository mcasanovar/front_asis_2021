...
import {
    getAbsensesByIdAndDateService,
    insertAbsenseService,
} from '../../services'
...
const AbsensesEmployeeView: React.FunctionComponent<IAbsensesEmployeeViewProps> = ({
    ...
    const [newDataAusence, setNewDataAusence] = useState<AusencesModel>(
        ausencesInitialization
    )
    ...
    const handleDataRenderDates = (value: Moment) => {
        const aux = absenses.filter(
            absense =>
                absense.fecha_inicio_ausencia === value.format(FORMAT_DATE)
        )
        if (aux) {
            return (
                <ul className="events">
    ...
    const handleDataRenderMonths = (value: Moment) => {
        const aux = absenses.filter(absense => {
            if (
                moment(absense.fecha_inicio_ausencia, FORMAT_DATE).format(
                    'MMMM'
                ) ===
                    moment(value.format(FORMAT_DATE), FORMAT_DATE).format(
                        'MMMM'
                    ) &&
                moment(absense.fecha_inicio_ausencia, FORMAT_DATE).format(
                    'YYYY'
    ...
    async function getAbsenses(
        ...
        setLoading(true)
        const aux: IResponseAusences = await getAbsensesByIdAndDateService(
            idEmployee,
            month,
            year
        )
                ...
                <Spin spinning={loading} size="large" tip="Cargando...">
                    <Form layout="vertical">
                        <Input.Group>
                                ...
                                <Col span={7}>
                                    <Form.Item label="Hora inicio">
                                        <TimePicker
                                            format="HH:mm"
                                            defaultValue={moment(
                                                newDataAusence.hora_inicio_ausencia,
                                                'HH:mm'
                                            )}
                                            style={{ width: '100%' }}
                                            value={moment(
                                ...
                                <Col span={7}>
                                    <Form.Item label="Hora fin">
                                        <TimePicker
                                            format="HH:mm"
                                            defaultValue={moment(
                                                newDataAusence.hora_fin_ausencia,
                                                'HH:mm'
                                            )}
                                            style={{ width: '100%' }}
                                            value={moment(
                            ...
                            <Row>
                                <Col span={24}>
                                    <Form.Item label="Seleccionar rango de fecha">
                                        <RangePicker
    ...
    return (
        <>
            <Spin
                spinning={loadingAbsenses}
                size="large"
                tip="Cargando ausencias..."
            >
                <Title>{`${currentDate.month.toUpperCase()} - ${
                    currentDate.year
                }`}</Title>
                {
                    <Row gutter={8} style={{ width: '100%' }}>
                        ...
                        >
                            <Title
                                level={4}
                            >{`${employeeSelected?.razon_social} - ${employeeSelected?.codigo}`}</Title>
                            <ButtonComponent
